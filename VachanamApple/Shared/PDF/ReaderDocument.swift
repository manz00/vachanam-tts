...
public class ReaderDocument: Identifiable, ObservableObject {
    ...
    public func updateParsedContent(
        ...
        if !chapterStartPages.isEmpty {
            self.tocItems = parsed.chapters.enumerated().map { (idx, ch) in
                let pIdx = idx < chapterStartPages.count ? chapterStartPages[idx] : idx
                return TOCItem(title: ch.title ?? "Section \(idx + 1)", pageIndex: pIdx)
            }
        } else {
            self.tocItems = parsed.chapters.enumerated().map { (idx, ch) in
