...
- **CoreGraphics & CoreText PDF Log Sanitization (`PDFLoggingSanitizer`)**:
  - **Elimination of Console Log Flooding**: Eliminates the flood of unbuffered low-level diagnostic messages written directly to `stderr` by Apple's CoreGraphics and CoreText engines when rendering, navigating, or extracting text from LaTeX-generated mathematical PDFs (e.g. `.notdef: no mapping.`, `mapsto: no mapping.`, `summationtext: no mapping.`, `parenleftbig: no mapping.`, `radicalbig: no mapping.`, `CGPDFImage(...): subsample_factor MISMATCH`, and `CTLD took ...s`).
  - **Asynchronous POSIX Pipe Interceptor**: Employs a non-blocking POSIX pipe redirect (`dup2` with `DispatchSourceRead` on a background utility queue) attached to `STDERR_FILENO` that pattern-matches and silently discards benign font-mapping and image-caching noise while forwarding all genuine assertions, fatal errors, and diagnostic messages to the original stderr.
