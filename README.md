...
- **Advanced Symbol-to-Speech & Math Normalization**:
  - `TextNormalizer.normalizeForSpeech` converts mathematical operators (`<` $\to$ `less than`, `>` $\to$ `greater than`, `×` $\to$ `times`, `÷` $\to$ `divided by`, `≠` $\to$ `is not equal to`, `≤` $\to$ `less than or equal to`, `≥` $\to$ `greater than or equal to`, `≈` $\to$ `approximately`, `∞` $\to$ `infinity`), vulgar fractions (`½` $\to$ `one half`, `¼` $\to$ `one quarter`), currencies (`$100` $\to$ `100 dollars`, `€`, `£`, `¥`), percentages (`25%` $\to$ `25 percent`), plus-minus (`±5` $\to$ `plus or minus 5`), temperatures and angles (`100°C` $\to$ `100 degrees Celsius`, `72°F` $\to$ `72 degrees Fahrenheit`, `90°` $\to$ `90 degrees`), and ampersands (`&` $\to$ `and`) into fluent speech while preserving sentence punctuation.
  - **Exponents & Multi-Digit Superscripts (`MathSpeechEngine`)**: Full integer exponent parsing ensures multi-digit powers (such as $10^{23}$, $10^{-34}$, $10^{-12}$, $x^{23}$, $y^{-12}$) are synthesized cleanly as holistic numbers (e.g., "ten to the power of twenty three", "ten to the minus thirty four", "x to the power of twenty three") rather than fragmented single-digit powers ("squared cubed" or "inverse squared").
...
- **🧠 Page Furniture & Reading Intelligence Engine (`PageFurnitureDetector`)**:
  - **Spatial Zone Filtering**: Distinguishes running headers (top 12%), running footers (bottom 10%), and footnotes (bottom 22%) relative to page coordinate dimensions.
  - **Paragraph Trailing Line & Sentence Continuation Protection**: Guarantees that body text and list items terminating near the bottom margin are never prematurely truncated or classified as running footers. Lines continuing an incomplete sentence from the previous line at normal line spacing (`verticalGap <= 1.8x line height`) or starting with a lowercase character are strictly preserved as narrative prose.
...
- **📜 Multi-Layout PDF Display Modes (`PDFDisplayLayoutMode`)**:
  ...
  - **Human-Centered Auto-Scroll & Free Reading Flow (`AutoScrollFollowMode`)**:
    ...
    - **State Loop & Cycle Isolation**: `PDFReaderView` isolates internal `PDFView` scroll offset and page changes from external navigation triggers via `lastHandledPageIndex`, `isHandlingPageChange`, `isProgrammaticScroll`, and touch-state tracking (`isUserScrolling`). Programmatic `uiView.go(to:)` calls from SwiftUI state bindings are dispatched asynchronously (`DispatchQueue.main.async`) and scrubber slider bindings decouple state mutation from render passes, eliminating SwiftUI `AttributeGraph: cycle detected` runtime warnings and unwanted programmatic snaps.
...
| **14. Developer Diagnostics** | Single-page JSON exporter, voice sandbox, menus | `Pass` | ⌘⌥D shortcut, 3-tab inspector, no duplicate menu |
...
7. **[AUD-07] SwiftUI AttributeGraph Cycle & Concurrency Elimination**:
   - Resolved `=== AttributeGraph: cycle detected ===` runtime warnings caused by synchronous `uiView.go(to:)` calls inside `PDFReaderView.updateUIView` which fed `PDFViewPageChanged` notifications directly back into SwiftUI during active render passes; dispatched navigation asynchronously to break the synchronous re-entrancy loop.
   - Refactored `ReaderScrubberBar` to decouple dragging from intermediate `@State` mutations and removed duplicate notification broadcasts.
   - Added asynchronous task lifecycle management (`documentInitTask` cancellation on re-entry and view dismissal) in `ReaderContainerView` to prevent redundant parallel parsing and repeated `loadDocument` invocations.
   - Pruned hidden overlay keyboard shortcut buttons to avoid shortcut conflicts with the primary scene `.commands`.
8. **[AUD-08] Initial Page Layout & Viewport Synchronization**:
   - Fixed issue where opening or restoring a document at a non-zero page showed the cover page while the scrubber displayed the restored page index.
   - Overrode `VachanamPDFView.layoutSubviews()` with an `onFirstLayout` hook to trigger programmatic page navigation once `PDFView` receives its non-zero bounds, eliminating failed zero-frame `go(to:)` invocations in `makeUIView`.
   - Set `isProgrammaticScroll = true` immediately when triggering programmatic navigation in `updateUIView` to prevent spurious `pageChanged` notifications from trampling the destination page before the asynchronous dispatch completes.
   - Unified `handleNextPage` and `handlePreviousPage` to guarantee discrete page stepping in continuous scroll modes while preserving distance-based scrolling for `Page Up` / `Page Down`.
9. **[AUD-09] Paragraph Trailing Line & Footer Disambiguation**:
   - Resolved bug where the last line of a paragraph or list item preceding a copyright notice near the bottom margin (e.g. `"row echelon form (the reduced row-echelon form is unnecessary here):"`) was misclassified as `.pageFooter` and omitted from TTS audio and sentence highlighting.
   - Tightened `footerBandYRatio` from `0.18` (18%) to `0.10` (bottom 10%) so that normal body paragraphs reaching the lower page margin are not swept into the footer zone.
   - In whole-document analysis, restricted `.pageFooter` classification exclusively to cross-page recurring strings, publication disclaimers, or page numbers; single non-recurring lines are preserved as narrative body text.
   - In `ParagraphDetector`, enforced that lines starting with a lowercase character or continuing an unpunctuated sentence from the preceding line at normal line spacing (`verticalGap <= 1.8x line height`) are strictly guarded against splitting into standalone page furniture.
10. **[AUD-10] Matrix Number Disambiguation & Equation Tag Unification**:
   - Fixed critical issue where numbers inside mathematical column vectors and matrices (e.g. entries `1`, `2`, `0`, `4`) were falsely classified as `.pageNumber` and silently discarded from audio speech due to an unconstrained `line.text.count <= 15` rule in `PageFurnitureDetector`. Confined page number detection strictly to `inHeaderZone || inFooterZone`.
   - Fixed matrix rows (e.g. `1 1 -1` and `2 1 -2`) being misidentified as section headings by enforcing that any heading must contain at least one alphabetic character.
   - Prevented equation tags (e.g. `(2.67)`, `(2.68)`, `(2.69)`) from being severed into isolated orphan sentences by protecting periods immediately preceding equation labels in `SentenceSegmenter`, keeping mathematical expressions and their reference labels unified as a single cohesive sentence and highlight span.
11. **[AUD-11] Number Verbalization & AttributeGraph Cycle Elimination**:
   - **Neural Phonemizer Number Verbalization**: Fixed fallback to Apple TTS (`The phonemizer returned no phonemes`) during neural synthesis of equation tags (`(2.78)`, `(2.77)`), matrices, and decimal coordinates. Implemented pre-G2P number verbalization in `KokoroMisakiPhonemizer` for decimals (`2.78` $\to$ `two point seven eight`), negative numbers, percentages, ordinals, attached letter-digits (`B1` $\to$ `B one`), and standalone integers (`1 0 0` $\to$ `one zero zero`).
   - **Empty / Punctuation Fragment Guard**: Added guards in `KokoroTTS.appendPreparedChunk` so trailing punctuation fragments produced during oversized chunk splitting are safely skipped rather than attempting CoreML prediction on empty phonemes.
   - **G2P Rule-Based Fallback**: Added a cross-platform fallback to `ruleBasedPhonemizeText` ensuring any speech-bearing text containing letters or numbers never produces empty phonemes.
   - **AttributeGraph Cycle Prevention**: Eliminated `=== AttributeGraph: cycle detected ===` runtime warnings during reader sentence tracking by guarding all `parent.currentPageIndex` assignments with `if self.parent.currentPageIndex != targetIndex` and restricting `TTSController.shared.currentSentenceViewRect` updates strictly to when `isReadingRulerEnabled` is active with double-checked equality guards.
12. **[AUD-12] Developer Mode, Page Structure Exporter & Menu Bar Fix**:
   - **Single-Page Structure Exporter**: Added `PageStructureExporter` providing comprehensive JSON serialization of document pages, layout blocks with coordinates (`[x, y, w, h]`), sentences, word tokens with exact bounding boxes, speech normalization (`TextNormalizer`), number verbalization (`KokoroMisakiPhonemizer`), and phonemes.
   - **Line & Paragraph-Level Download/Export**: Added paragraph diagnostic models with character counts, word counts, block types, phoneme token lengths, and duration bucket threshold checks (`> 128` tokens), with one-click "Copy JSON" and temporary file sharing/download (`ShareLink`).
   - **Voice Testing Sandbox Harness**: Created `VoiceTestingSandboxView` enabling live multi-stage inspection (Input $\to$ Text Normalization $\to$ Number Verbalization $\to$ Phonemization $\to$ CoreML Neural Synthesis) with audio duration, Real-Time Factor (RTF) calculation, in-sandbox audio playback, voice selector (`af_heart`, `af_bella`, `am_adam`, `bf_alice`, etc.), and comprehensive diagnostic report generation.
   - **Developer Inspector Sheet**: Created `DeveloperInspectorView` featuring 3 tabs (Page Structure, Paragraphs & Lines, and Voice Sandbox) with search/filter capabilities, raw JSON inspection sheet, and seamless "Test in Sandbox" button to jump from any page paragraph directly into the voice synthesis harness.
   - **Developer Mode Toggle & Discovery**: Added `DeveloperModeManager` persisted in `UserDefaults` (`vachanam_developer_mode_enabled`), exposed in `ReadingSettingsView`, with a dedicated cyan wrench toolbar button in `ReaderContainerView` when enabled.
   - **Duplicate "View" Menu Bar Fix**: Replaced top-level `CommandMenu("View")` with native `CommandGroup(after: .toolbar)` in `VachanamApp.swift`. This integrates zoom and layout commands directly into macOS's standard native View menu without generating a redundant second "View" menu, and binds `Developer Inspector...` to `⌘ + ⌥ + D`.
13. **[AUD-13] Sentence Flow & Cadence Restoration, Sandbox Audio Download, Line-Break Dehyphenation & Publisher Disclaimer Isolation**:
   - **Sentence Cadence & Flow Restoration (`SentenceSegmenter`)**: Reverted artificial substitution of periods with commas before equation tags. This preserves authentic terminal falling pitch contours in Kokoro TTS, eliminating unnatural pitch spikes and erratic prosodic fluctuations between adjacent sentences.
